...
const starcro = require('starcro-elements');
...
function migrateSecrets(password) {
	...
	oldConfig.forging.secret.forEach(secret => {
		...
		oldConfig.forging.delegates.push({
			encryptedPassphrase: starcro.default.cryptography.stringifyEncryptedPassphrase(
				starcro.default.cryptography.encryptPassphraseWithPassword(
					secret,
					password
				)
			),
			publicKey: starcro.default.cryptography.getPrivateAndPublicKeyFromPassphrase(
				secret
			).publicKey,
		});
	});
