...
class Round {
	 ...
	 */
	deleteRoundRewards() {
		this.scope.library.logger.debug(`Deleting rewards for round ${this.scope.round}`);
	...
	applyRound() {
		...
		const delegates = self.scope.backwards ? self.scope.roundDelegates.reverse() : self.scope.roundDelegates;
		...
		let delegatesLength = self.scope.roundDelegates.length;
		for (let i = 0; i < delegatesLength; i++) {
			delegate = self.scope.roundDelegates[i];
			changes = roundChanges.at(i, delegatesLength);
			...
			p = new Promise((resolve, reject) => {
				self.scope.modules.accounts.mergeAccountAndGet(accountData, (err, account) => {
					if (err) {
						reject(err);
					} else {
						resolve(account);
		...
		changes = roundChanges.at(remainderIndex, delegatesLength);

		// Apply fees remaining to chosen delegate
		if (changes.feesRemaining > 0) {
			const feesRemaining = this.scope.backwards ? -changes.feesRemaining : changes.feesRemaining;
			...
			if (!self.scope.backwards) {
				roundRewards[roundRewards.length - 1].fees = new bignum(roundRewards[roundRewards.length - 1].fees).plus(feesRemaining).toString();
			}

			queries.push(p);
		}

		this.scope.library.logger.trace("Starcronus start");
		const genesisAccountData = {
			publicKey: constants.starcronusPublicKey,
			balance: self.scope.backwards ? -roundChanges.roundFees : roundChanges.roundFees,
			u_balance: self.scope.backwards ? -roundChanges.roundFees : roundChanges.roundFees,
			round: self.scope.round,
			fees: self.scope.backwards ? -roundChanges.roundFees : roundChanges.roundFees,
			rewards: 0
		};

		this.scope.library.logger.trace("Starcronus Data", genesisAccountData);
		
		p = new Promise((resolve, reject) => {
			self.scope.modules.accounts.mergeAccountAndGet(genesisAccountData, (err, account) => {
				if (err) {
					reject(err);
				} else {
					resolve(account);
		...
		queries.push(p);
		this.scope.library.logger.trace("Starcronus end");
