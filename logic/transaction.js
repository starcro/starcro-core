...
class Transaction {
	...
	getBytes(transaction, skipSignature, skipSecondSignature) {
		...
		try {
			...
			byteBuffer = new ByteBuffer(1 + 4 + 32 + 32 + 8 + 8 + 64 + 64 + assetSize, true);
	...
	checkBalance(amount, balance, transaction, sender) {
		if( transaction.type === 1){
			...
			const exceededBalance = new bignum(sender[balance].toString()).lessThan(amount);
			const exceeded = transaction.blockId !== this.scope.genesisblock.block.id && exceededBalance;

			return {
				exceeded,
				error: exceeded ? [
					'Account does not have enough XSC:', sender.address,
	...
	verify(transaction, sender, requester, checkExists, cb, tx) {
		...
		if(transaction.type !== 1) {
			if ([exceptions.genesisPublicKey.mainnet, exceptions.genesisPublicKey.testnet].indexOf(sender.publicKey) !== -1
				&& transaction.blockId !== this.scope.genesisblock.block.id) {
				if (transaction.type !== 8 && transaction.type !== 1) {
					return setImmediate(cb, 'Invalid sender. Can not send from genesis account');
				}
			}
		}

		//Check sender is genesis account
		...
		const multisignatures = transaction.type !== 1 ? sender.multisignatures || [] : [];
		...
		if (transaction.type === 1 || transaction.type === 8) {
			const fee = __private.types[transaction.type].calculateFee.call(this, transaction, sender);
			...
			const fee = __private.types[transaction.type].calculateFee.call(this, transaction, sender) || false;
		...
		const amount = new bignum(transaction.amount.toString()).plus(transaction.fee.toString());
		const senderBalance = this.checkBalance(amount, 'balance', transaction, sender);
		...
		// Check timestamp
		if (transaction.timestamp < INT_32_MIN || transaction.timestamp > INT_32_MAX) {
		...
		const verifyTransactionTypes = (transaction, sender, tx, verifyTransactionTypesCb) => {
