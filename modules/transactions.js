...
const Reward = require('../logic/reward.js');
...
class Transactions {
	constructor(cb, scope) {
		...
		__private.assetTypes[transactionTypes.REWARD] = library.logic.transaction.attachAssetType(
			transactionTypes.REWARD, new Reward(library.logger, library.schema)
		);
...
__private.list = function(filter, cb) {
	...
	const processParams = function(value, field) {
		...
		if (_.includes(['fromUnixTime', 'toUnixTime'], field)) {
			// Starcro epoch is 1464109200 as unix timestamp
	...
	library.db.transactions.countList(Object.assign({}, {
		...
		const transactions = rawTransactionRows.map(rawTransactionRow => library.logic.transaction.dbRead(
				_.merge(rawTransactionRow, assetRowsByTransactionId[rawTransactionRow.t_id])));
...
__private.getQueryNameByType = function(type) {
	...
	queryNames[transactionTypes.REWARD] = 'getTransferByIds';
...
Transactions.prototype.onBind = function(scope) {
	...
	__private.assetTypes[transactionTypes.REWARD].bind(scope.accounts);
};
