...
Verify.prototype.verifyBlock = function(block) {
	...
	result.errors.reverse();
	if (result.verified) {
		library.logger.info(`Verify->verifyBlock succeeded for block ${block.id} at height ${block.height}.`);
	} else {
		library.logger.error(`Verify->verifyBlock failed for block ${block.id} at height ${block.height}.`,	result.errors);
	}
	...
	async.series(
		{
			...
			applyBlock(seriesCb) {
				...
				// * The check against database state passed (for instance sender has enough XSC, votes are under 975, etc...)
