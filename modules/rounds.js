...
Rounds.prototype.backwardTick = function(block, previousBlock, done, tx) {
	...
	scope.finishRound = (prevRound === round && nextRound !== round) || (block.height === 1 || block.height === 975);
	...
	function backwardTick(tx) {
		...
		return round.mergeBlockGenerator().then(() => (scope.finishRound ? round.backwardLand() : round));
	}
	...
	async.series([
		...
		function(cb) {
			// Perform round tick
			tx.task(backwardTick).then(() => setImmediate(cb)).catch(err => {
				library.logger.error(err.stack);
...
Rounds.prototype.tick = function(block, done, tx) {
	...
	// Establish if finishing round or not
	scope.finishRound = round !== nextRound || (block.height === 1 || block.height === 975);
	...
	async.series([
		...
		// Perform round tick
		function(cb) {
			(tx || library.db).task(Tick).then(() => setImmediate(cb)).catch(err => {
				library.logger.error(err.stack);
				return setImmediate(cb, err);
			});
		},
...
__private.sumRound = function(scope, cb, tx) {
	...
	(tx || library.db).rounds.summedRound(scope.round, constants.activeDelegates).then(rows => {
		const rewards = [];
