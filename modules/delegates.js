...
__private.decryptPassphrase = function(encryptedPassphrase, password) {
	return starcro.cryptography.decryptPassphraseWithPassword(
		starcro.cryptography.parseEncryptedPassphrase(encryptedPassphrase),
		password
	);
};
...
__private.loadDelegates = function(cb) {
	...
	library.logger.info(`Loading ${encryptedList.length} delegates using encrypted passphrases from config`);
	...
	async.eachSeries(
		...
		(encryptedItem, seriesCb) => {
			...
			const keypair = library.ed.makeKeypair(crypto.createHash('sha256').update(passphrase, 'utf8').digest());

			if (keypair.publicKey.toString('hex') !== encryptedItem.publicKey) {
				return setImmediate(seriesCb, `Invalid encryptedPassphrase for publicKey: ${encryptedItem.publicKey}. Public keys do not match`);
			}
 ...
 * @todo Add description for the return value
 */
Delegates.prototype.updateForgingStatus = function(publicKey, password, forging, cb) {
	const encryptedList = library.config.forging.delegates;
	const encryptedItem = encryptedList.find(item => item.publicKey === publicKey);
	...
	if (encryptedItem) {
		try {
			passphrase = __private.decryptPassphrase(encryptedItem.encryptedPassphrase, password);
		} catch (e) {
			return setImmediate(cb, 'Invalid password and public key combination');
		}

		keypair = library.ed.makeKeypair(crypto.createHash('sha256').update(passphrase, 'utf8').digest()		);
	} else {
...
Delegates.prototype.getForgers = function(query, cb) {
	...
	// For example: last block height is 975 (still round 1, but already finished), then we want the list for round 2 (height 102)
