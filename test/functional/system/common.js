...
function beforeBlock(type, cb) {
	...
	before(
		'init sandboxed application, credit account and register dapp',
			...
			application.init(
				{ sandbox: { name: `starcro_test_${type}` } },
				(err, library) => {
					cb(library);
					done();
				}
...
function loadTransactionType(key, account, dapp, secondPassphrase, cb) {
	...
	switch (key) {
		case 'SEND':
			...
			break;
		case 'SIGNATURE':
			transaction = starcro.transaction.registerSecondPassphrase({
				passphrase: account.passphrase,
				secondPassphrase: account.secondPassphrase,
			});
			break;
		case 'DELEGATE':
			transaction = starcro.transaction.registerDelegate({
				passphrase: accountCopy.passphrase,
				secondPassphrase: accountCopy.secondPassphrase,
		...
		case 'VOTE':
			...
			break;
		case 'MULTI':
			transaction = starcro.transaction.registerMultisignature({
				passphrase: accountCopy.passphrase,
				secondPassphrase: accountCopy.secondPassphrase,
			...
			break;
		case 'DAPP':
			transaction = starcro.transaction.createDapp({
				passphrase: accountCopy.passphrase,
				secondPassphrase: accountCopy.secondPassphrase,
