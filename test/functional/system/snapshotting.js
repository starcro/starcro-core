...
const elements = require('starcro-elements').default;
...
describe('snapshotting', () => {
	...
	localCommon.beforeBlock('starcro_functional_snapshotting', lib => {
		library = lib;
		...
		before(() => {
			...
			return (
				Promise.mapSeries([...Array(99)], () => {
					...
					// Forge 1 block with transaction to reach height 975
					.then(() => {
					...
					// Forge 975 block with transaction to reach height 202
					.then(() => {
						return Promise.mapSeries([...Array(975)], () => {
							return addTransactionsAndForgePromise(library, [], 0);
						});
					})
					.then(() => {
