...
describe('system test (type 0) - address collision', () => {
	...
	var publicKeys = [
		starcro.cryptography.getPrivateAndPublicKeyFromPassphrase(
	...
	var firstTransaction = starcro.transaction.transfer({
		amount: 10 * constants.normalizer,
		passphrase: collision.passphrases[0],
		recipientId: accountFixtures.genesis.address,
	});

	var secondTransaction = starcro.transaction.transfer({
		amount: 10 * constants.normalizer,
		passphrase: collision.passphrases[1],
		recipientId: accountFixtures.genesis.address,
	});

	var firstTransactionWithData = starcro.transaction.transfer({
		amount: 10 * constants.normalizer,
		passphrase: collision.passphrases[0],
		recipientId: accountFixtures.genesis.address,
	...
	var secondTransactionWithData = starcro.transaction.transfer({
		amount: 10 * constants.normalizer,
		passphrase: collision.passphrases[1],
		recipientId: accountFixtures.genesis.address,
	...
	before(done => {
		var creditTransaction = starcro.transaction.transfer({
	...
	it('both passphrases should have the same address', done => {
		expect(starcro.cryptography.getAddressFromPublicKey(publicKeys[0])).to.equal(
			starcro.cryptography.getAddressFromPublicKey(publicKeys[1])
		);
		done();
	});
