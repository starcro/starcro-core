...
describe('inert transactions', () => {
	...
	const transferInertTransaction = starcro.transaction.transfer({
		recipientId: recipientAccount.address,
		amount: 1000000000 * 100,
		passphrase: senderAccount.passphrase,
	});

	const voteInertTransaction = starcro.transaction.castVotes({
		passphrase: recipientAccount.passphrase,
		votes: [`${accountFixtures.existingDelegate.publicKey}`],
	});

	const delegateInertTransaction = starcro.transaction.registerDelegate({
		passphrase: recipientAccount.passphrase,
	...
	describe('send funds to account', () => {
		before(done => {
			const transferTransaction = starcro.transaction.transfer({
				recipientId: recipientAccount.address,
