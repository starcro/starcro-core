...
describe('validateOwnChain', () => {
	...
	localCommon.beforeBlock(
		'starcro_functional_validate_own_chain_full_two_rounds',
		lib => {
			library = lib;
	...
	describe('forge 3 rounds (975 blocks) with version = 0', () => {
		before(() => {
			// Set current block version to 0
			blockVersion.currentBlockVersion = 0;

			// Not consider the genesis block
			return Promise.mapSeries([...Array(975 * 3 - 1)], () => {
		...
		describe('increase block version = 1 and exceptions for height = 975', () => {
			...
			it('there should be no error during chain validation', () => {
				expect(library.logger.info).to.be.calledWith(
					'Finished validating the chain. You are at height 975.'
				);
				return expect(validateOwnChainError).to.be.eql(null);
			});

			it('blockchain should be at height 975', () => {
				const lastBlock = library.modules.blocks.lastBlock.get();
				return expect(lastBlock.height).to.eql(975);
			});
