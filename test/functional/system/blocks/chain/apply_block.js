...
describe('system test (blocks) - chain/applyBlock', () => {
	...
	beforeEach('send funds to accounts', done => {
		...
		const fundTrsForAccount1 = starcro.transaction.transfer({
			amount: transferAmount,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: blockAccount1.address,
		});

		const fundTrsForAccount2 = starcro.transaction.transfer({
			amount: transferAmount,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: blockAccount2.address,
		});

		const fundTrsForAccount3 = starcro.transaction.transfer({
			amount: transferAmount,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: poolAccount3.address,
		});

		const fundTrsForAccount4 = starcro.transaction.transfer({
			amount: transferAmount,
			passphrase: accountFixtures.genesis.passphrase,
	...
	describe('applyBlock', () => {
		...
		beforeEach('create block', done => {
			blockTransaction1 = starcro.transaction.registerDelegate({
			...
			blockTransaction2 = starcro.transaction.registerDelegate({
		...
		describe('undoUnconfirmedList', () => {
			...
			beforeEach('with transactions in unconfirmed queue', done => {
				transaction3 = starcro.transaction.registerSecondPassphrase({
					passphrase: poolAccount3.passphrase,
					secondPassphrase: poolAccount3.secondPassphrase,
				});

				transaction4 = starcro.transaction.registerSecondPassphrase({
