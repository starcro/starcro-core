...
describe('POST /api/transactions (type 0) transfer funds', () => {
	...
	describe('schema validations', () => {
		...
		it('with lowercase recipientId should fail', () => {
			...
			transaction.id = starcro.transaction.utils.getTransactionId(transaction);
		...
		it('when sender has no funds should fail', () => {
			...
			return sendTransactionPromise(
				...
				expect(res.body.message).to.be.equal(
					`Account does not have enough XSC: ${account.address} balance: 0`
				);
				badTransactions.push(transaction);
			});
		});
		...
		it('using entire balance should fail', () => {
			...
			return sendTransactionPromise(
				...
				expect(res.body.message).to.match(
					/^Account does not have enough XSC: [0-9]+L balance: /
				);
				badTransactions.push(transaction);
			});
		});
