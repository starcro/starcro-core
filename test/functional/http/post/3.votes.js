...
describe('POST /api/transactions (type 3) votes', () => {
	...
	*/
	...
	before(() => {
		var transactions = [];
		var transaction1 = starcro.transaction.transfer({
			amount: 1000 * constants.normalizer,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: delegateAccount.address,
		});
		var transaction2 = starcro.transaction.transfer({
			amount: constants.fees.vote,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: accountMinimalFunds.address,
		});
		var transaction3 = starcro.transaction.transfer({
			amount: 1000 * constants.normalizer,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: accountFixtures.existingDelegate.address,
		});
		var transaction4 = starcro.transaction.transfer({
			amount: 1000 * constants.normalizer,
			passphrase: accountFixtures.genesis.passphrase,
			recipientId: accountMaxVotesPerTransaction.address,
		});
		var transaction5 = starcro.transaction.transfer({
			amount: 1000 * constants.normalizer,
			passphrase: accountFixtures.genesis.passphrase,
		...
		return Promise.all(promises)
			...
			.then(() => {
				...
				var transaction = starcro.transaction.registerDelegate({
	...
	describe('transactions processing', () => {
		it('using invalid publicKey should fail', () => {
			transaction = starcro.transaction.castVotes({
				passphrase: delegateAccount.passphrase,
		...
		it('upvoting with no funds should fail', () => {
			...
			return sendTransactionPromise(
				...
				expect(res.body.message).to.be.equal(
					`Account does not have enough XSC: ${
		...
		it(`upvoting ${
			...
			var transaction1 = starcro.transaction.castVotes({
				passphrase: accountMaxVotesPerAccount.passphrase,
				votes: delegatesMaxVotesPerAccount.slice(0, 33).map(delegate => {
					return `${delegate.publicKey}`;
				}),
			});
			var transaction2 = starcro.transaction.castVotes({
				passphrase: accountMaxVotesPerAccount.passphrase,
				votes: delegatesMaxVotesPerAccount.slice(33, 66).map(delegate => {
					return `${delegate.publicKey}`;
				}),
			});
			var transaction3 = starcro.transaction.castVotes({
				passphrase: accountMaxVotesPerAccount.passphrase,
				votes: delegatesMaxVotesPerAccount.slice(66, 99).map(delegate => {
					return `${delegate.publicKey}`;
				}),
			});
			var transaction4 = starcro.transaction.castVotes({
				passphrase: accountMaxVotesPerAccount.passphrase,
