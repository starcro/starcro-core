...
random.account = function() {
	...
	account.publicKey = starcro.cryptography.getKeys(account.passphrase).publicKey;
	account.address = starcro.cryptography.getAddressFromPublicKey(account.publicKey);
	account.secondPublicKey = starcro.cryptography.getKeys(
...
// Returns an random basic transfer transaction to send 1 XSC from genesis account to a random account
random.transaction = function(offset) {
	return starcro.transaction.transfer({
		amount: 1,
