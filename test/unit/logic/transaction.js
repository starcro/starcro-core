...
describe('transaction', () => {
	...
	before(done => {
		...
		application.init(
			{ sandbox: { name: 'starcro_test_logic_transactions' } },
			(err, scope) => {
	...
	describe('getBytes', () => {
		...
		it('should return same result of getBytes using /logic/transaction and starcro-elements package (without data field)', () => {
			var transactionBytesFromLogic = transactionLogic.getBytes(
				validTransaction
			);
			var transactionBytesFromLiskJs = starcro.transaction.utils.getTransactionBytes(
				validTransaction
			);
	...
	describe('checkBalance', () => {
		...
		it('should return error when sender has insufficiant balance', () => {
			...
			return expect(res.error).to.include('Account does not have enough XSC:');
		});
	...
	describe('verify', () => {
		function createAndProcess(transactionData, sender, cb) {
			...
			var transaction = starcro.transaction.transfer(transferObject);
		...
		it('should return error when transaction amount is invalid', done => {
			// starcro-elements cannot create a transaction with higher amount than max amount. So, this test needs to use hardcoded transaction object
			var transaction = {
				type: 0,
		...
		it('should return error when account balance is less than transaction amount', done => {
			...
			createAndProcess(transactionDataClone, sender, (err, transaction) => {
				transactionLogic.verify(transaction, sender, null, null, err => {
					expect(err).to.include('Account does not have enough XSC:');
					done();
				});
			});
		});
