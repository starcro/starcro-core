	...
	beforeEach(() => {
		...
		accountsMock = {
			generateAddressByPublicKey: sinonSandbox
				.stub()
				.returns(starcro.cryptography.getKeys(randomUtil.password()).publicKey),
	...
	describe('calculateFee', () => {
		it('should return correct fee based on formula for 1 keysgroup', () => {
			transaction.asset.multisignature.keysgroup = [
				`${starcro.cryptography.getKeys(randomUtil.password()).publicKey}`,
			];
			return expect(
	...
	describe('objectNormalize', () => {
		...
		it('should return transaction when created using createMultisignature', () => {
			var keysgroup = Array(...Array(10)).map(() => {
				return `${starcro.cryptography.getKeys(randomUtil.password()).publicKey}`;
			});
