		...
		IF is_mainnet > 0 THEN
			...
			WITH
				...
				round AS (
					...
					FROM blocks b
					WHERE ceil(b.height / 975::float)::int = 27040
				),
				-- Calculating total fees of round, apply exception fees bonus
				fees AS (SELECT sum(fees) + fees_bonus AS total, floor((sum(fees) + fees_bonus) / 975) AS single FROM round GROUP BY fees_bonus),
			...
			FROM (
				SELECT
					...
					round.reward,
					-- Round
					ceil(round.height / 975::float)::int as round,
					-- Delegate public key
					round."publicKey"
