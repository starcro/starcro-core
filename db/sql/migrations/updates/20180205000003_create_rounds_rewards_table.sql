		...
		LOOP
			...
			WITH
				...
				round AS (
					SELECT
						...
						b.reward AS reward
					FROM blocks b
					WHERE ceil(b.height / 975::float)::int = row.round
				),
				-- Calculating total fees of round
				fees AS (SELECT sum(fees) AS total, floor(sum(fees) / 975) AS single FROM round),
			...
			INSERT INTO rounds_rewards
				SELECT
					...
					(fees.single + (CASE WHEN last."publicKey" = round."publicKey" AND last.timestamp = round.timestamp THEN (fees.total - fees.single * 975) ELSE 0 END)) AS fees,
					-- Block reward
					round.reward,
					-- Round
					ceil(round.height / 975::float)::int,
					-- Delegate public key
					round."publicKey"
